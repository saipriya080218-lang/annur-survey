...
function App() {
  ...
  const handleSubmit = () => {
    ...
    if (Object.keys(answers).length < surveyData.length) {
      alert(`Please answer all ${surveyData.length} questions before submitting.`);
    } else {
      alert("Survey Submitted Successfully!");
      console.log("Patient Data:", answers);
    }
  };

  return (
    <div style={{ padding: '20px', maxWidth: '600px', margin: 'auto', textAlign: 'center' }}>
      <h1 style={{ color: '#003B7A' }}>Annur Vision Centre</h1>
      ...
      {/* This section automatically creates all 29 question blocks */}
      {surveyData.map((q) => (
        <div key={q.id} style={{ textAlign: 'left', border: '1px solid #ddd', padding: '15px', borderRadius: '10px', marginBottom: '15px' }}>
          <p><strong>Q{q.id}. {q[language]}</strong></p>
